...
module.exports = class User {
  ...
  static fetchTeamByLeader(id_user) {
    return db.execute("SELECT id_team FROM team WHERE id_leader = ?", [
      id_user,
    ]);
  }
  ...
  static approve(id_user) {
    return db.execute(`UPDATE user SET status = 'active' WHERE id_user = ?`, [
      id_user,
    ]);
  }
  ...
  static findBySlackUser(slack_user) {
    return db.execute("SELECT * FROM user WHERE slack_user = ?", [slack_user]);
  }

  static updateSlack(id_user, slack_user) {
    return db.execute("UPDATE user SET slack_user = ? WHERE id_user = ?", [
      slack_user,
      id_user,
    ]);
  }
  ...
  static updatePassword(id_user, hashedPassword) {
    return db.execute("UPDATE user SET password = ? WHERE id_user = ?", [
      hashedPassword,
      id_user,
    ]);
  }
